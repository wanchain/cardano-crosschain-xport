...
contract WmbGateway is AccessControl, Initializable, ReentrancyGuard, IWmbGateway, IWmbConfig {
    ...
    function dispatchMessageNonEvm(uint256 toChainId, bytes memory to, uint256 gasLimit, bytes calldata data) external payable nonReentrant returns (bytes32 messageId) {
        ...
        messageGasLimit[messageId] = gasLimit;
        emit MessageDispatchedNonEvm(messageId, msg.sender, toChainId, to, gasLimit, data);
    }
    ...
    function receiveMessageNonEvm(
        ...
        _receiveMessageNonEvm(
            messageId,
            ReceiveMsgDataNonEvm(
                sourceChainId,
                sourceContract,
                targetContract,
                messageData,
    ...
    function _receiveMessageNonEvm(
        bytes32 messageId,
        ReceiveMsgDataNonEvm memory data
    ) internal {
        if(messageExecuted[messageId]) {
            revert MessageIdAlreadyExecuted({messageId: messageId});
        }
