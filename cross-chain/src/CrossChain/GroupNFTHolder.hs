  ...
  where 
    ...
    outputHasToken = -- True
      let params = groupInfoParams ownOutput
          ...
          bSingleAsset = isSingleAsset (V2.txOutValue ownOutput) groupNftSymbol groupNftName
          isRightOwner = 
            case V2.txOutAddress ownOutput of
              Address (Plutus.ScriptCredential s) _ -> s == (ValidatorHash ver)
              _ -> False
      in  bSingleAsset && isRightOwner
    ...
    checkNewDatum =
      let 
        ...
        !checkParamsLen = 
          if action == 0 then True
          else (length oldParams) == (length newParams)
      in (paramsDiff oldParams newParams action) && checkParamsLen
